...
export default function Example() {
  ...
  return (
    <div className="isolate bg-white px-6 py-20 w-10/12 mx-auto">
        <Link to={'/'} className={`text-xl max-lg:text-lg max-md:text-[1rem] max-sm:text-sm flex gap-x-2 items-center text-blue-600 cursor-pointer`}>
            <TbArrowBackUp className='max-sm:bg-white max-sm:text-xl max-sm:p-[3px] flex items-center justify-center max-sm:rounded-full' /> <span className='max-sm:hidden'>Go back</span>
        </Link>
        <div
            aria-hidden="true"
        ...
        </div>
        {
            loading ? (<Loader />) : (<>
                <div className="mx-auto max-w-2xl text-center">
                    <h2 className="text-4xl font-bold tracking-tight text-balance text-gray-900 sm:text-5xl">Login</h2>
                </div>
                <form onSubmit={submitHandler} className="mx-auto mt-16 max-w-xl sm:mt-20">
                    ...
                    <div className="mt-10">
                        <button
                            type="submit"
                            className="block w-full rounded-md bg-indigo-600 px-3.5 py-2.5 text-center text-lg font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600"
                        >
                            Login
                        </button>
                    </div>
                ...
                <div className='text-sm mx-auto mt-2 font-[500] w-full text-black flex items-center justify-center gap-x-1'>
                    {`Don't have account?`}<Link to={'/signup'}><span className='text-blue-800 cursor-pointer font-semibold'>Signup</span></Link>
                </div>
            </>)
        }
    </div>
