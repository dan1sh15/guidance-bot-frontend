...
export default function UdemyCourses() {
    ...
    const fetchCourses = async () => {
        setLoading(true);
        const url = import.meta.env.VITE_API_FLASK_URL + '/recommended-udemy-courses/' + job_role;
            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                {courses.map((courses, idx) => (
                    <div key={idx+1} className='flex flex-col gap-y-7 bg-white px-5 py-5 justify-between rounded-lg capitalize text-lg shadow-xl '>
                        ...
                        <p className='text-slate-600'><strong>Instructor: </strong>{courses.instructor}</p>
                        <p className='text-slate-600'><strong>Titles: </strong>{courses.title}</p>
                    </div>
                ))}
                </div>
            </div>
