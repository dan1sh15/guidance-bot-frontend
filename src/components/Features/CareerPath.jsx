...
export default function CareerPaths() {
    ...
    const fetchCourses = async () => {
        setLoading(true);
        const url = import.meta.env.VITE_API_FLASK_URL + '/career-path/' + job_role;
    ...
    loading ? (<div className='py-24 sm:py-32'><Loader /></div>) : (
        <div className="py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <h2 className="text-4xl font-semibold tracking-tight text-pretty text-gray-900 sm:text-5xl">Career Path</h2>
                <p className="mt-2 text-lg/8 text-gray-600">Carrer Path for {job_role}</p>
                </div>
                ...
                {careerPath.map((career, idx) => (
                    <div key={idx+1} className='flex flex-col gap-y-7  bg-white px-5 py-5 justify-between rounded-lg capitalize text-lg shadow-xl '>
                        <p className='text-2xl font-bold text-center'>{career.current_role}</p>
                        <p className='text-slate-600'><strong>Experience: </strong>{(career.experience_required)}</p>
                        <p className='text-slate-600'><strong>Next step: </strong>{career.next_step}</p>
                        <p className='text-slate-600'><strong>Salary: &#8377;</strong>{career.salary}</p>
                    </div>
                ))}
                </div>
            </div>
